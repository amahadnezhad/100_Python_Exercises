"""
Exercise 100
Question: Create a web app with Flask that lets the user submit a username and a password.
          The app then checks if the username exists in a text file,
          and it also checks if the password contains at least one number,
          one uppercase letter, and it is at least 5 characters long.
...
html = """
  		      <div class="form">
                  <form action="{{url_for('sent')}}" method="POST">
  			        <input title="Your email address will be safe with us." placeholder="Enter username" type="text" 
  			            name="username" required> <br>
  			        <input title="Your email address will be safe with us." placeholder="Enter password" type="text" 
